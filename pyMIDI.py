...
class MidiFile:
    ...
    def __init__(self, midi_file, verbose=False, debug=False):
        ...
        try:
            ...
            self.readEvents()
            print(self.key_press_count, "notes processed. Your MIDI survived!")
            self.clean_notes()
            self.success = True
        except Exception as e:
            print(f"Couldn't process MIDI file. Maybe it's haunted? Error: {e}")
            self.bytes = None
            self.notes = []
            self.success = False
        finally:
        ...
        # Only adjust last note if MIDI was processed successfully
        if self.success and self.notes:
            self.notes[len(self.notes) - 1][0] = 1.00
    ...
    def readLength(self):
        if self.bytes is None:
            print("No MIDI data to read. Did you break something?")
            return 0
        contFlag = True
        length = 0
        while contFlag and self.itr < len(self.bytes):
    ...
    def readMidiTrackEvent(self, length):
        if self.bytes is None:
            print("No MIDI data to read. Skipping track event.")
            return
        self.log("TRACKEVENT")
        self.deltaTime = 0
        start = self.itr
        ...
        while length > self.itr - start and continueFlag:
            ...
            if self.itr >= len(self.bytes):
                print("Reached end of MIDI data unexpectedly. That's awkward.")
                break
...
def get_file_choice():
    ...
    if not midList:
        print("No MIDI files detected. Please add MIDI files to the 'midi' folder. (Don't worry, I'll wait.)")
        return None
    ...
    try:
        ...
        print("That wasn't a valid choice. Try again, but with feeling!")
        return None
...
def main():
    if len(sys.argv) > 1:
        ...
        if not os.path.exists(midi_file):
            print(f"Error: file not found '{midi_file}'. Maybe check your spelling?")
            return 1

        if not (".mid" in midi_file or ".mid" in midi_file.lower()):
            print(f"'{midi_file}' has an incorrect file extension. Only .mid files, please!")
    ...
    try:
        ...
        print("Something went wrong while processing your MIDI.\nIf it starts playing backwards, call an exorcist.\nError:\n", e)
        return 1
